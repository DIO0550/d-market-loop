...
`steps/commit.md` の規則に従ってこのグループの変更をコミットする。
`commitPrefix` は通常 `fix` を使う（既存バグに対するレビュー指摘の修正のため）。
**1 グループが commit.md の分割基準（異なる関心事・異なるレイヤー・リファクタ同居等）
にヒットしたら、そのグループ内でさらに複数コミットに分割する**。
