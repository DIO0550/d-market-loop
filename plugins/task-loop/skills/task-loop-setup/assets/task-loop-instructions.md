...
**重要: 1回の実行で処理するタスクは1つだけ。** 1タスク完了（または失敗）したら終了すること。

## 前提条件

実行前に以下を確認する。満たされていない場合はエラーメッセージを出して停止する。
...
処理開始前に `task-loop-state.json` を確認する。`status: "in_progress"` のタスクがあれば:

1. `git branch -a` でタスクブランチの存在を確認
2. `gh pr list --head {branch}` でPRの存在を確認
3. PRが存在する場合 → レビュー待ちステップから再開
4. PRが存在しない場合 → ブランチに未コミットの変更があるか確認
   - 変更あり → コミットステップから再開
   - 変更なし → 実装ステップから再開
...
4. `reviewMaxWaitMinutes` を超えた場合:
   ...
   - `autoMergeWithoutReview` が `false` → タスクを保留して終了
...
5. 修正回数をカウントし、`maxFixIterations` に達した場合:
   - タスクの状態を `"needs_manual_review"` としてエラー終了
