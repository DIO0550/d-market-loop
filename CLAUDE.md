...
外部シェルスクリプト (`run-loop.sh`) が Claude のセッションをモード別に起動し、レビュー待ちのポーリングはシェル側が担当する。タスクの状態は `task-loop-state.json` で管理し、`tasks/` ディレクトリ間のファイル移動（todo → processing → done/failed）で進捗を追跡する。
